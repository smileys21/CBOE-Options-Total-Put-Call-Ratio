...
(marketdata.theocc.com/weekly-volume-reports)

Parses TOTAL CALLS and TOTAL PUTS contract counts from weekly reports.
First run: ~260 requests for 5yr history (~90 seconds). Cached after that.
"""
...
# ── OCC Weekly Volume Parsing ────────────────────────────────────────────────

def parse_int(s):
    """Parse OCC formatted integers like '"133,935,382"' into int."""
    return int(s.strip().strip('"').replace(",", ""))


def parse_weekly_report(text):
    """
    Parse OCC weekly equity options volume report.
    Finds TOTAL CALLS and TOTAL PUTS rows and extracts total contracts.
    Returns (total_calls, total_puts) or (None, None) on failure.
    """
    total_calls = None
    total_puts = None
    ...
    for line in text.split("\n"):
        ...
        if line_stripped.startswith("TOTAL CALLS"):
            parts = line_stripped.split(",")
            if len(parts) >= 2:
                try:
                    total_calls = parse_int(parts[1])
                except (ValueError, IndexError):
                    pass

        elif line_stripped.startswith("TOTAL PUTS"):
            parts = line_stripped.split(",")
            if len(parts) >= 2:
                try:
                    total_puts = parse_int(parts[1])
                except (ValueError, IndexError):
                    pass
...
def extract_week_ending(text):
    """Extract the week-ending date from the report header."""
    match = re.search(r"Week Ending\s+(\d{2}/\d{2}/\d{4})", text)
    if match:
        return datetime.strptime(match.group(1), "%m/%d/%Y")
    return None


def fetch_occ_week(friday_date):
    """Fetch OCC weekly equity options volume for a given Friday date."""
    ...
    try:
        ...
        calls, puts = parse_weekly_report(text)
        if calls and puts and calls > 0:
            week_end = extract_week_ending(text) or friday_date
            return {"date": week_end, "calls": calls, "puts": puts, "ratio": puts / calls}
        return None
    except Exception:
        return None
...
def get_fridays(start, end):
    """Generate list of Fridays between start and end dates."""
    fridays = []
    d = start
...
def build_dataset(years_needed, progress_bar=None, status_text=None):
    """Download OCC weekly data, using cache for known weeks."""
    cached = load_cache()
    end = datetime.now()
    ...
    for i, friday in enumerate(missing):
        if progress_bar:
            progress_bar.progress((i + 1) / len(missing),
                                  text=f"Fetching week ending {friday.strftime('%Y-%m-%d')} ({i+1}/{len(missing)})")

        result = fetch_occ_week(friday)
        if result:
            new_rows.append({"Date": pd.Timestamp(result["date"]),
            ...
            if fails > 8:
                if status_text:
                    status_text.text("OCC stopped responding — using available data...")
                break
...
def plot_cpc(df, years, ma_short, ma_long, show_raw):
    ...
    fig.update_layout(**PLOT_LAYOUT,
                      title=dict(text="CBOE Total Put/Call Ratio ($CPC · weekly)",
...
st.markdown("*Weekly equity options volume from OCC · Put contracts ÷ Call contracts*")
st.divider()
...
if data is not None and not data.empty:
    ...
    ma_s = plot_data["MA_Short"].iloc[-1] if not np.isnan(plot_data["MA_Short"].iloc[-1]) else 0
    ma_l = plot_data["MA_Long"].iloc[-1] if not np.isnan(plot_data["MA_Long"].iloc[-1]) else 0
    ...
    st.markdown(
        ...
        f'Source: OCC weekly equity options volume · Latest: {data.index.max().strftime("%Y-%m-%d")} · '
    ...
    with st.expander("📖 Notes"):
        st.markdown(f"""
**Data:** Weekly total put and call contract volumes from OCC (Options Clearing Corporation), equity options only. 
The ratio = total puts ÷ total calls for each week.

**vs StockCharts $CPC:** StockCharts shows the *daily* CBOE-only ratio. This shows *weekly* across all exchanges cleared by OCC. 
The levels and trend will be similar but not identical — weekly data smooths out daily spikes, and OCC covers all 16 exchanges, not just CBOE.

**Cache:** Data is stored at `~/.cpc_cache/cpc_weekly.csv`. First load downloads ~{int(cpc_years * 52)} weeks (~{int(cpc_years * 52 / 3)} seconds). 
Subsequent loads only fetch new weeks.

**MA defaults:** 4-week and 10-week MAs roughly correspond to StockCharts' 20-day and 50-day MAs on the daily chart.
""")
else:
    st.error("No data available. Check internet or click Force refresh.")
